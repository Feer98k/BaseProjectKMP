package com.f98k.baseproject.data.constants

enum class RemoteConfigEnum(val value: String) {
    GENERAL_MATCHES_HML("general_matches_hml"),
    BOTTOM_BAR_LIST("bottom_bar_items"),

    PREMIER_LEAGUE("premier_league"),
    LA_LIGA("la_liga"),
    SERIE_A("serie_a"),
    BUNDESLIGA("bundesliga"),
    LIGUE_1("ligue_1"),
    BRASILEIRAO("brasileirao"),
    MLS("mls"),
    LIGA_PORTUGAL("liga_portugal"),
    EREDIVISIE("eredivisie"),
    SAUDI_PRO_LEAGUE("saudi_pro_league"),
    ARGENTINE_PRIMERA("argentine_primera"),
    J1_LEAGUE("j1_league"),
    CHINESE_SUPER_LEAGUE("chinese_super_league"),
    SCOTTISH_PREMIERSHIP("scottish_premiership"),
    BELGIAN_PRO_LEAGUE("belgian_pro_league"),

    UEFA_CHAMPIONS_LEAGUE("uefa_champions_league"),
    UEFA_EUROPA_LEAGUE("uefa_europa_league"),
    COPA_LIBERTADORES("copa_libertadores"),
    AFC_CHAMPIONS_LEAGUE("afc_champions_league"),

    FIFA_WORLD_CUP("fifa_world_cup"),
    UEFA_EURO("uefa_euro"),
    COPA_AMERICA("copa_america"),
    GOLD_CUP("gold_cup");
}